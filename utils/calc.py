...
def simulate_sync(unit, bases, steel_storage=None, alum_storage=None):
    ...
    if steel_storage is None and alum_storage is None:
        ...
        while current_time + EPSILON < time:
            needed_steel = steel_cost - current_steel
            needed_alum = aluminium_cost - current_alum
            ...
            time_steel = max(0.0, needed_steel / pps)
            time_alum = max(0.0, needed_alum / pps)
            wait_time = max(time_steel, time_alum)

            if wait_time <= EPSILON:
                break
            if current_time + wait_time > time:
                break
